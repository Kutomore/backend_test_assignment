...
describe GetCarRecommendationsService do
  ...
  describe '#self.perform' do
    ...
    let(:ai_response) {
      Car.all.shuffle.sample(10).map { |car|
        {
          "car_id" => car.id,
    ...
    context 'when it gets filtered by max price' do
      let(:params) { { user_id: user.id, price_max: 100000 } }
      let!(:excluded_car) { FactoryBot.create(:car, price: 100001) }
    ...
    context 'when it gets filtered by min price' do
      let(:params) { { user_id: user.id, price_min: 10000 } }
